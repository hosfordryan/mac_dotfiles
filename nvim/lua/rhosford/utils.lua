...
function M.findup(filenames, filesystem_type)
...
local function run_command(command, opts)
  ...
  if results.return_code ~= 0 then
    vim.notify(command .. ', stderr:\n' .. results.stderr, vim.log.levels.ERROR, { title = cmd .. ' returned ' .. results.return_code })
  end
...
function M.is_running_location(location)
  if location == 'laptop' and string.find(M.run_command('uname').stdout, 'Darwin') then
    return true
  end
